...
const TV_ENDPOINTS = {
  ...
  upcoming: `${BASE_URL}/tv/upcoming`,
  genres: `${BASE_URL}/genre/tv/list`,
...
async function performSearch(query) {
  ...
  isSearchActive = true;
  const searchResults = await fetchFromTMDB(TV_ENDPOINTS.search, true, query);
  ...
  searchResults.forEach((tv) => {
    if (tv.media_type === "tv") {
      searchResultsContainer.appendChild(createMovieCard(tv));
    }
  });
...
async function fetchShows(page = 1) {
  try {
    let endpoint = TV_ENDPOINTS[currentSort] || TV_ENDPOINTS.popular;
    let url = `${endpoint}?api_key=${TMDB_API_KEY}&language=en-US&page=${page}`;

    if (currentGenre !== "all") {
      // Use the 'discover' endpoint when filtering by genre
      url = `${BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&language=en-US&page=${page}&with_genres=${currentGenre}`;
    }
...
async function populateGenreFilter() {
  try {
    const response = await fetch(
      `${TV_ENDPOINTS.genres}?api_key=${TMDB_API_KEY}`
    );
    const data = await response.json();
...
function toggleMyList(item) {
  ...
  if (itemInList) {
    ...
    myList.push({
      id: item.id,
      media_type: item.media_type || "tv", // Set the media type to 'tv' if it's not provided
...
function createMovieCard(item) {
  ...
  card.dataset.mediaType = "tv"; // Default to 'movie' if not specified
...
async function displayMovies(page = 1, append = false) {
  ...
  showsGrid.innerHTML = "";
  showsGrid.appendChild(loadingIndicator);
